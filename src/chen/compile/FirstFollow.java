 ...
 * 4、最后用end结束 实现功能： 1、标准文法输入
 * 5、求First集和First求解算法 3、求Follow集合Follow集求解算法
 * 6、随便多个语句输入 
 * 7、忽略空格的影响
 * 8、兼容两种推导符的输入和中英文输入一撇符和或连接符 
 * 9、简单输入错误判断功能 某一输入实例：
 */
// E->TE'
// E'->+E|ε
...
public class FirstFollow {
	public ArrayList<String[]> in = new ArrayList<String[]>();// 实现了可变长度文法接收，在这存放的是拆分后最简单的文法，也是由用户输入
	...
	public FirstFollow(String main) {
		...
		while (!sline.startsWith("end")) {
			...
			sline = sc.nextLine();//一旦调用这个方法,控制台就阻塞,直至在控制台输入end后回车
...
//		}
	...
	public void process(String firstORfollow) {
		for (int i = 0; i < in.size(); i++) {//i是字符串数组的个数
			boolean bool = true;
			for (int j = 0; j < i; j++)
			...
			for (int i = 0; i < in.size(); i++) {
				...
				if (s.equals(one[0])) {
					if (track1.substring(0, track1.length() - 9).indexOf(
							"First(" + s + ")") >= 0)// 假如在查找过程嵌套了这步，证明进入了无限循环，不需再找，此路径无结果// 有点要注意一下，本来一开始就把第一个开始推导符的First路径放进去了的，所以要避开这一次，不然已开始就结束了
						;//System.out.println("AAAAAAAAAA");
